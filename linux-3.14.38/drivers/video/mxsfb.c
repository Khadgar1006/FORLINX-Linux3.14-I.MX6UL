...
static int mx6_clk_from_boot = 0;
...
static int mxsfb_init_fbinfo(struct mxsfb_info *host)
{
	...
	var->yres_virtual = var->yres * fb_info->fix.ypanstep;	
...
#else
	//mxsfb_restore_mode(host);
        if(mxsfb_restore_mode(host) && mx6_clk_from_boot)
            memset((char *)fb_info->screen_base, 0, fb_info->fix.smem_len);
#endif
	return 0;
}
